          ...
          <div class="modal-body" :key="componentKey">
            <form class="form-group" @submit.prevent="submitAccomodationObject">
              ...
              <div class="mb-3 form-group">
                 ...
                 <div :class="['mb-3', 'form-group', error.input.includes('cost') ? 'error' : '']">
                  <label>Cost of accommodation</label>
                  <input type="text" class="form-control" name="cost" v-model="newData.cost">
              </div>
              <span class="error-text" v-if="error.input.includes('cost')">
      ...
      if (this.error.input.length === 0) {
        ...
        AccommodationService.postAccomodation(accomodation)
          .then((response) => {
            ...
            Swal.fire({
              title: 'Saved',
              text: 'Accomodation has been added',
              icon: 'success',
    ...
    deleteAccomodationItem(id) {
      AccommodationService.deleteAccomodation(id)
        .then((response) => {
          Swal.fire({
            title: 'Deleted',
            text: 'Accomodation has been Permanently deleted',
            icon: 'success',
            confirmButtonColor: '#045C44',
          });
    ...
    confirmDelete(id) {
      Swal.fire({
        ...
        if (result.isConfirmed) {
          ...
          Swal.fire({
            title: 'Cancelled',
            text: 'Accomodation is still available',
            icon: 'info',
            confirmButtonColor: '#045C44',
          });
        }
